...
df_units = pd.read_excel('output/jaren/2024-2025/8_analyse_units.xlsx', sheet_name='Analyse')

df_clusters_100m = pd.read_excel('output/16_jaren_samen.xlsx', sheet_name='Straal 100m')
...
df_clusters_100m = df_clusters_100m.rename(
    columns={
        'llngroepen_diff': 'gesplitste_leerlingengroepen'
    }
)

df_clusters_adres = pd.read_excel('output/16_jaren_samen.xlsx', sheet_name='Adres')
...
df_clusters_adres = df_clusters_adres.rename(
    columns={
        'llngroepen_diff': 'gesplitste_leerlingengroepen'
    }
)
